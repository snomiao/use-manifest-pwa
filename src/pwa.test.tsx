...
describe("PWA Integration", () => {
  const mockManifest: Manifest &
    Required<Pick<Manifest, "name" | "icons" | "start_url">> = {
    name: "Test PWA",
    icons: [
      {
        src: "icon-192.png",
  ...
  test("manifest contains required icons for installability", async () => {
    ...
    const has192 = manifestContent.icons.some(
      (icon) => icon.sizes === "192x192"
    );
    const has512 = manifestContent.icons.some(
  ...
  test("updates both manifest and service worker when changed", () => {
    ...
    expect(document.head.querySelector('link[rel="manifest"]').href).toContain(
      "blob:"
    );
